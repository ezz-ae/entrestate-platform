...
import { Tabs, TabsList, TabsTrigger } from "@/data-scientist/components/ui/tabs"
import { VegaChart } from "@/data-scientist/components/vega-chart"
import type { VisSpec, DatasetProfile, DrillDownOption } from "@/data-scientist/lib/types"
import { createNotebookEntry, saveNotebookEntry } from "@/data-scientist/lib/notebook"
...
export function ChartExpandedModal({
  ...
  const generateInsights = async () => {
    ...
    try {
      const response = await fetch("/api/data-scientist/llm/drilldown", {
        method: "POST",
  ...
  const loadInitialRecommendations = async () => {
    ...
    if (defaultField) {
      const defaultOption: DrillDownOption = {
        type: "breakdown",
        label: `Segment by ${defaultField}`,
        description: "Starter segment suggestions",
        field: defaultField,
      }
  ...
  const loadDrillDownCharts = async (option: DrillDownOption | null, tabType: typeof activeTab) => {
    ...
    try {
      ...
      const response = await fetch("/api/data-scientist/recommend/drilldown", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          datasetId,
      ...
      if (!response.ok) throw new Error("Failed to load next view")
      const data = await response.json()
      setDrillDownCharts(data.charts || [])
    } catch (error) {
      console.error("Failed to load next view:", error)
      setDrillDownCharts([])
    } finally {
      setIsLoadingDrillDown(false)
    }
  ...
  const handleSelectDrillDownChart = (newChart: VisSpec) => {
    ...
    const fallbackLabel = newChart.intent?.columns?.[0] || "deep dive"
    const newClause = { 
  ...
  const handleSaveSnapshot = () => {
    ...
    const entry = createNotebookEntry({
      datasetId,
      title: currentChart.title || "Market snapshot",
      summary: insights.summary || "Saved market snapshot.",
      highlights: insights.keyFindings || [],
      chart: currentChart,
    })
    saveNotebookEntry(entry)
    setSavedSnapshotId(entry.id)
  }
        ...
        <div className="flex items-center gap-2 flex-1 justify-center overflow-x-auto px-4">
          {clauses.map((clause, i) => (
            <Badge 
              ...
              className={`flex items-center gap-1 flex-shrink-0 ${i >= 2 ? "border-foreground/20 bg-muted/40 text-foreground" : ""}`}
            >
              {clause.label}
              {i >= 2 && (
                <button 
      ...
      <div className="flex h-[calc(100vh-57px)]">
        ...
        <div className="w-[380px] border-r border-border/50 flex flex-col bg-card/30">
          <div className="px-4 py-3 border-b border-border/50">
            <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Selected view</h3>
          </div>
            ...
            {isLoadingInsights ? (
              ...
              <div>
                <div className="flex items-start gap-2">
                  <Sparkles className="h-3.5 w-3.5 text-foreground mt-0.5 flex-shrink-0" />
                ...
                {insights.keyFindings.length > 0 && (
                  <>
                    ...
                    >
                      ...
                      {insightsExpanded ? "Hide highlights" : "Show highlights"}
                    </button>
                ...
                <div className="mt-3">
                  <Button
                    ...
                    disabled={Boolean(savedSnapshotId)}
                  >
                    <Bookmark className="mr-2 h-4 w-4" />
                    {savedSnapshotId ? "Saved to notebook" : "Save to notebook"}
                  </Button>
                </div>
              </div>
            ) : (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-3 border-b border-border/50 flex items-center justify-between">
            <div>
              <h3 className="font-medium">Next views</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Pick a view to go deeper.</p>
            </div>
            ...
            {isLoadingDrillDown ? (
              ...
              <div className="space-y-4">
                {drillDownCharts.map((drillChart) => (
                  <button
                    key={drillChart.id}
                    className="w-full rounded-[6px] border border-border/60 bg-card/50 p-4 text-left hover:border-foreground/30 hover:bg-muted/40 hover:shadow-md transition-all group overflow-hidden"
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0 py-2">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {activeTab === "breakdown" && "Segmented by market signal"}
                          {activeTab === "slice" && "Focused on a specific market slice"}
                          {activeTab === "compare" && "Compare two market signals"}
                        </p>
                        <div className="mt-3 flex items-center text-sm text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                          Explore this view
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center max-w-sm">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {activeTab === "breakdown" && "Segment this view by a market signal."}
                    {activeTab === "slice" && "Focus on one slice of the market."}
                    {activeTab === "compare" && "Compare this signal with another market factor."}
                  </p>
                </div>
              </div>
            )}
