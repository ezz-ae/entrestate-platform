...
**Answer**: Entrestate operates under a strict "The AI Proposes; Human Applies" boundary, enforced by the **Spend Governor (`spend-authority.ts`)**. The AI can analyze campaign performance, calculate optimal budget allocations, and propose campaign modifications, but it has zero write-authority on external marketing APIs. No marketing budget can be committed or modified on Meta or Google Ads without a recorded human approval written to the ledger — the ledger entry is the control, and it names the person, the rule and the moment. If an automated ad set crosses its cost-per-lead threshold, the system triggers the Spend Governor to programmatically scale down or pause the campaign, ensuring absolute capital safety.
...
Instead of discarding "idleness" as trash, the system treats it as a signal. We calculate a user's **Re-Engagement Velocity** based on repeated refocusing events (e.g., returning to a JVC apartment tab after 2 hours of inactivity). If this repeated behaviour is validated, the system records it as **verified intent**: it lifts the inbound baseline and is attached as evidence to every convergence escalation. It does not by itself produce a Rate 8 — that comes from intent convergence or the broker's own five-star call, which is the more defensible claim. A static, unrefocused background tab contributes nothing. This shields our database and ad custom-audience seeds from telemetry pollution, delivering a clean, highly testable event structure.
...
#### Q7.1: Most SaaS companies charge high monthly subscription fees per user seat. Why does Entrestate include its core CRM, integrations, and Terminal with the account, and how does your App Store model make commercial sense?
...
Entrestate aligns its commercial success directly with the transactional velocity of its clients. Market discovery comes with the account. Our entire core substrate—the unified CRM, automation builders, database integrations, and Market Terminal—is included, with no seat fee. The platform is fully white-labeled; brokers customize their palettes and logos, removing the Entrestate brand beyond the login screen. Inviting teammates to a company account carries no per-seat charge.
