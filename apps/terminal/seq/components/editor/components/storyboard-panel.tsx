    ...
    const handleEnhancePrompt = useCallback(
      async (panelId: string, prompt: string) => {
        ...
        try {
          ...
          onUpdatePanel(panelId, { status: "error", error: "Refinement failed" })
          toast.error("Could not refine brief")
        }
      },
      [onUpdatePanel, toast],
    )
    ...
    const handleEnhanceMaster = useCallback(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          toast.error("Could not refine master brief")
          return
        }
        const data = await response.json()
        ...
        setIsEnhancingMaster(false)
        toast.success("Master brief refined")
      } catch (error) {
        setIsEnhancingMaster(false)
        toast.error("Could not refine master brief")
      }
