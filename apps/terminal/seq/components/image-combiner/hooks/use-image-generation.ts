...
export function useImageGeneration({
  ...
  const generateImage = async (options?: GenerateImageOptions) => {
    ...
    if (currentMode === "image-editing" && !effectiveUseUrls && !effectiveImage1) {
      onToast("Please add at least one reference image", "error")
      return
    }
    if (currentMode === "image-editing" && effectiveUseUrls && !effectiveImage1Url) {
      onToast("Please add at least one image link", "error")
      return
    }
    if (!effectivePrompt.trim()) {
      onToast("Please enter a brief", "error")
      return
    }
  ...
  const loadGeneratedAsInput = async () => {
    ...
    try {
      ...
      const file = new File([blob], "created-image.png", { type: "image/png" })
