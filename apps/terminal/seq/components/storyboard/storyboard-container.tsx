...
export function StoryboardContainer({
  ...
  const generateVideo = async (id: string) => {
    ...
    try {
      ...
      if (result.data?.video?.url) {
        ...
        throw new Error("No clip output in response")
      }
    } catch (error: any) {
      console.error("Clip build failed:", error)
      updatePanel(id, {
        isGenerating: false,
        error: error.message || "Failed to build clip",
      })
      toast.error("Clip build failed")
    }
  }
  ...
  const generateAll = async () => {
    ...
    if (pendingPanels.length === 0) {
      toast.info("No ready panels to build (check shot notes)")
      return
    }

    toast.info(`Building ${pendingPanels.length} panels...`)
  ...
  const loadDemoData = () => {
    ...
    toast.success(`Sample notes loaded for ${matchedCount} panel${matchedCount !== 1 ? "s" : ""}.`)
  }
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center text-center bg-card">
        ...
        <p className="text-xs text-muted-foreground max-w-xs mb-4">
          Add images from above to build a clip storyboard.
        </p>
      </div>
    )
  }
      ...
      <div className="flex flex-col overflow-hidden bg-card" style={{ width: `${leftWidth}%`, minWidth: "280px" }}>
        <div className="flex-shrink-0 h-12 px-4 flex items-center justify-between border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-foreground">Clip Storyboard</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {panels.length > 0 && panels.every((p) => !p.prompt) && (
            <div className="text-xs p-3 flex items-start gap-2 rounded-lg bg-muted text-muted-foreground">
              ...
              <div>
                ...
                <p>
                  Your selected panels are ready. You can now add shot notes manually, use the "Load Sample Data"
                  button to pre-fill example notes, or enhance notes using the master description.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="rounded-lg bg-muted p-4 space-y-2">
            ...
            <Textarea
              ...
              placeholder="Describe the overall story, style, or context (e.g. 'A waterfront launch at sunset, warm cinematic lighting'). This will be used to refine individual shot notes."
          ...
          <div className="rounded-lg p-4 bg-muted">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] text-muted-foreground font-medium">Render speed</label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="fast">Quick Cut</SelectItem>
                    <SelectItem value="standard">Studio</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-background">
        <div className="flex-shrink-0 h-12 px-4 flex items-center justify-between border-b border-border">
          ...
          <span className="text-xs text-muted-foreground">
            {panels.filter((p) => p.videoUrl).length}/{panels.length} clips built
          </span>
        </div>
