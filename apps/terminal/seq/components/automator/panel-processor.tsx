...
export function PanelProcessor({ masterUrl, masterPrompt, panelCount, storageMode, onComplete }: PanelProcessorProps) {
  ...
  const loadDemoPanels = () => {
    ...
    toast.success(`Loaded ${demoPanelUrls.length} sample panels`)
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold mb-2 text-[var(--text-primary)]">Preparing panels</h2>
        <p className="text-[var(--text-secondary)]">
          Splitting the master board into {panelCount} clean panels for review.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <Card className="p-4 bg-[var(--surface-2)] border-[var(--border-default)]">
          <div className="aspect-[3/2] relative rounded-lg overflow-hidden">
            <Image src={masterUrl || "/seq-poster.svg"} alt="Master" fill className="object-cover" />
          ...
          <div className="grid grid-cols-3 gap-2">
            {Array.from({ length: panelCount }).map((_, i) => (
              ...
              >
                {panels[i] ? (
                  <>
                    <Image src={panels[i] || "/seq-poster.svg"} alt={`Panel ${i}`} fill className="object-cover" />
                    {status === "complete" && (
