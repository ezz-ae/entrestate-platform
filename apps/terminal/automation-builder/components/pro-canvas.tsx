...
const STORAGE_KEY = "automation-builder-pro-workflow"
...
const getDefaultNodeData = (type: string) => {
  switch (type) {
    ...
    case "prompt":
      return { content: "Add a clear instruction for the automation." }
...
export function ProCanvas({ editable, onToggleEdit }: ProCanvasProps) {
  ...
  useEffect(() => {
    const fetchInitialAgent = async () => {
      try {
        ...
        if (!listResponse.ok) {
          throw new Error("Failed to fetch automation list");
        }
        const automations = await listResponse.json();

        if (agents && agents.length > 0) {
          const firstAutomationId = automations[0].id;
          const automationResponse = await fetch(`/api/automation-builder/${firstAutomationId}`);
          if (!automationResponse.ok) {
            throw new Error(`Failed to fetch automation ${firstAutomationId}`);
          }
          const automation = await automationResponse.json();

          if (automation?.workflowGraph?.nodes && automation?.workflowGraph?.edges) {
            setNodes(automation.workflowGraph.nodes);
            setEdges(automation.workflowGraph.edges);
            setCurrentAutomationId(automation.id);
          } else {
            console.warn("First automation found but no workflowGraph data.");
          }
        } else {
          console.log("No automations found, starting with initial workflow.");
          setNodes(initialNodes);
          setEdges(initialEdges);
        }
      } catch (error) {
        console.error("Error fetching initial automation workflow:", error);
  ...
  const handleExportWorkflow = useCallback(async () => {
    const agentName = window.prompt("Enter a name for your automation:");
    if (!agentName) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Automation workflow saved successfully:", savedAutomation);
        alert(`Automation '${agentName}' saved!`);
