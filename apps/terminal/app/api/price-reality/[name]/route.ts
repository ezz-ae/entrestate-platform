...
import { getPriceRealityByProjectName } from "@/lib/decision-infrastructure"
...
export async function GET(request: Request, context: { params: Promise<{ name: string }> }) {
  ...
  try {
    ...
    const data = await getPriceRealityByProjectName(decodeURIComponent(name))
    return NextResponse.json({ ...data, requestId })
  } catch (error) {
    return NextResponse.json(
      { error: getPublicErrorMessage(error, "Failed to load price reality."), requestId },
      { status: 500 },
    )
  }
}
