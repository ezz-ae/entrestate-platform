...
import { getDeveloperReliabilityByName } from "@/lib/decision-infrastructure"
...
export async function GET(request: Request, context: { params: Promise<{ name: string }> }) {
  ...
  try {
    ...
    const data = await getDeveloperReliabilityByName(decodeURIComponent(name))
    return NextResponse.json({ ...data, requestId })
  } catch (error) {
    return NextResponse.json(
      { error: getPublicErrorMessage(error, "Failed to load developer reliability."), requestId },
      { status: 500 },
    )
  }
}
