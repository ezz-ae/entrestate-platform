...
export async function POST(request: Request) {
  try {
    ...
    // Return fallback
    return NextResponse.json({
      summary: "This view shows the market spread.",
      keyFindings: ["The market reveals patterns worth exploring."],
      drillDownOptions: [],
      relatedQuestions: ["What factors might influence this market spread?"],
    })
  }
}
...
function buildDrillDownPrompt(visSpec: VisSpec, profile: ProfileSummary): string {
  ...
  return `You are a real estate analyst. Analyze this view and suggest meaningful drill-down explorations.
...
- Available segments for breakdown: ${categoricalCols.map((c) => `${c.name} (${c.cardinality} values)`).join(", ") || "none"}
- Available numeric signals: ${numericCols.map((c) => c.name).join(", ") || "none"}

Respond with a JSON object in this exact format:
{
  "summary": "A 1-2 sentence summary of what this view reveals",
  "keyFindings": ["finding 1", "finding 2", "finding 3"],
  "drillDownOptions": [
    {
      "type": "breakdown",
      "label": "Break down by SegmentName",
      "description": "See how the distribution varies across different SegmentName values",
      "field": "SegmentName"
    },
    {
      "type": "filter",
    ...
    {
      "type": "compare",
      "label": "Compare with AnotherSignal",
      "description": "See how this relates to another market signal",
      "field": "AnotherSignal",
      "secondaryField": "OptionalSecondSignal"
    }
  ],
...
- For "filter" type: suggest filtering to interesting subsets (use actual signal names from the inventory)
- For "compare" type: suggest scatter plots or cross-comparisons with other signals
- Provide 3-5 drill-down options that would genuinely help understand the market
...
function parseDrillDownResponse(
  ...
  try {
    ...
    return {
      ...
      keyFindings: ["The market shows an interesting distribution."],
