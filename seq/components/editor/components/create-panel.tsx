      ...
      <div className="flex gap-3 p-2.5">
        ...
        <div className="relative w-16 h-12 rounded bg-black shrink-0 overflow-hidden border border-[var(--border-default)]">
          {item.type === "video" ? (
            ...
            <img src={item.url || "/seq-poster.svg"} alt="" className="w-full h-full object-cover" />
          )}
...
export const CreatePanel = memo(function CreatePanel({
  ...
  return (
    <PanelContainer>
      <PanelHeader title="Create clip" onClose={onClose}>
        {isGenerating && (
          <div className="flex items-center gap-2 px-2 py-1 bg-[var(--tertiary-muted)] rounded border border-[var(--tertiary)]/30">
            ...
            <span className="text-[10px] font-medium text-[var(--tertiary)]">Building...</span>
          </div>
        )}
      </PanelHeader>
      ...
      <PanelContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider">Creative brief</label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Describe the clip you want to build..."
              rows={3}
          ...
          <Section title="Reference Image (Optional)" defaultOpen={false}>
            ...
            {imagePreview ? (
              <div className="relative rounded-lg overflow-hidden border border-[var(--border-default)]">
                <img src={imagePreview || "/seq-poster.svg"} alt="Reference" className="w-full h-32 object-cover" />
                <button
                  type="button"
