            ...
            {(isGeneratingImage || isGeneratingVideo || isEnhancing) && (
              <div className="flex items-center gap-1 ml-2">
                ...
                <span className="text-[8px] text-[var(--accent-text)]">
                  {isGeneratingImage ? "Building image..." : isGeneratingVideo ? "Building clip..." : "Refining..."}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="p-3 flex flex-col gap-3">
          ...
          <div className="relative">
            <textarea
              ...
              placeholder={
                isTransition
                  ? "Describe the motion between start and end frames (e.g. 'Morph smoothly')..."
                  : "Describe the shot..."
              }
            ...
            <button
              ...
              title="Refine brief using master context"
            >
              {isEnhancing ? (
                <div className="animate-spin w-3 h-3 border-2 border-[var(--accent-primary)] border-t-transparent rounded-full" />
              ) : (
          ...
          {isTransition ? (
            ...
            <div className="flex gap-2 items-center">
              ...
              <div className="relative flex-1 aspect-video bg-[var(--surface-1)] rounded border border-[var(--border-default)] overflow-hidden group/media">
                {panel.imageUrl ? (
                  <>
                    <img src={panel.imageUrl || "/seq-poster.svg"} className="w-full h-full object-cover" />
                    <button
              ...
              <div className="relative flex-1 aspect-video bg-[var(--surface-1)] rounded border border-[var(--border-default)] overflow-hidden group/media">
                {panel.linkedImageUrl ? (
                  <>
                    <img src={panel.linkedImageUrl || "/seq-poster.svg"} className="w-full h-full object-cover" />
                    <button
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    {isGeneratingImage ? (
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[8px] text-neutral-500">Building...</span>
                      </div>
                    ) : (
                      <span className="text-[9px] text-neutral-700">No visual</span>
                    )}
                  </div>
                )}
                    ...
                    <div className="flex gap-2">
                      {panel.prompt && (
                        ...
                        >
                          ...
                          <span className="text-[8px] font-medium">Build</span>
                        </button>
                      )}
                      <button
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    {isGeneratingVideo ? (
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[8px] text-[var(--accent-text)]">Building...</span>
                      </div>
                    ) : (
                      <span className="text-[9px] text-neutral-700">No clip</span>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="flex justify-between items-center pt-1">
            <div className="text-[9px] text-neutral-600 font-mono">
              {videoConfig.useFastModel ? "Quick Cut" : "Studio"}
            </div>
            ...
            <div className="flex gap-2">
              {!panel.videoUrl && !isGeneratingImage && !isGeneratingVideo && (
                ...
                >
                  ...
                  {isTransition ? "Build transition" : "Build clip"}
                </button>
              )}
