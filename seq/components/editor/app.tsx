...
function createDemoData() {
  const mediaPanels = DEMO_FINAL_SEQUENCE.panels.filter((p) => p.videoUrl)
  const initialMedia: MediaItem[] = mediaPanels.map((p, i) => ({
    id: `media-${i}`,
    url: p.videoUrl,
    prompt: p.prompt,
    duration: p.duration,
  ...
  const initialStoryboard: StoryboardPanel[] = DEMO_FINAL_SEQUENCE.panels.map((p, i) => ({
    ...
    mediaId: p.videoUrl ? `media-${i}` : undefined,
...
function EditorContent() {
  ...
  const initialImageUrl = searchParams.get("media") || undefined
