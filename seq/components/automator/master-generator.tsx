...
export function MasterGenerator({ onGenerate, onLoadDemo, initialPrompt, initialImageUrl }: MasterGeneratorProps) {
  ...
  const [mode, setMode] = useState<"generate" | "upload">(initialImageUrl ? "upload" : "generate")
  ...
  useEffect(() => {
    ...
    if (initialImageUrl) {
      setGeneratedUrl(initialImageUrl)
      setMode("upload")
      analyzeImage(initialImageUrl)
    }
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto">
      <div className="text-center space-y-2 mb-8">
        <h2 className="text-section-title tracking-tight">Build a master storyboard</h2>
        <p className="text-body">Start with a creative brief or upload a master frame to begin the flow.</p>
      </div>
        ...
        {!generatedUrl ? (
          <Tabs defaultValue="generate" className="w-full" onValueChange={(v) => setMode(v as any)}>
            <TabsList className="grid w-full grid-cols-2 mb-6 bg-[var(--surface-3)]">
              <TabsTrigger value="generate">Create from brief</TabsTrigger>
              <TabsTrigger value="upload">Upload board</TabsTrigger>
            </TabsList>

            <TabsContent value="generate" className="space-y-4">
              <div className="space-y-2">
                <Label>Describe the story</Label>
                <Textarea
                  placeholder="Example: A waterfront launch story, showing the arrival, amenities, and sunset skyline across 6 frames..."
            ...
            <TabsContent value="upload" className="space-y-6">
              ...
              >
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <h3 className="font-medium text-lg text-[var(--text-primary)]">Click to upload a master board</h3>
                  <p className="text-sm text-[var(--text-secondary)]">Supports JPG, PNG, WEBP (rec. 3:2 ratio)</p>
                </div>
              </div>

              <div className="space-y-2">
                <Label>Optional: Style notes for refinement</Label>
                <Input
                  placeholder="Add visual notes to keep the style consistent..."
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="relative aspect-3/2 rounded-xl overflow-hidden border border-[var(--border-emphasis)]">
              <Image src={generatedUrl || "/seq-poster.svg"} alt="Storyboard Master" fill className="object-cover" />
            ...
            <div className="flex gap-4">
              ...
              >
                ...
                Approve & Prepare panels
              </Button>
            </div>
            <p className="text-xs text-center text-[var(--text-muted)]">
              {analyzedCount
                ? `Approving will split this board into ${analyzedCount} panels for review.`
                : "Approving will split this board into panels for review."}
            </p>
          </div>
        )}
      </Card>
