...
export function TransitionGenerator({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const systemPrompt =
        ...
        "The frames should be clearly distinct keyframes that the engine can interpolate between."
  ...
  const loadDemoExtractedPanels = () => {
    ...
    setProgress(100)
    setStatus("complete")
    toast.success(`Loaded ${demoPanelUrls.length} sample transition frames`)
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="text-center space-y-3 mb-8">
        <h2 className="text-3xl font-bold tracking-tight text-[var(--text-primary)]">Build transition frames</h2>
        <p className="text-[var(--text-secondary)] max-w-2xl mx-auto">
          Your main storyboard contains intermediate transition states. Create clean start/end frames for smooth
          transitions.
        </p>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          <Card className="p-4 bg-[var(--surface-2)] border-[var(--border-default)]">
            ...
            <div className="aspect-3/2 relative rounded-lg overflow-hidden border border-[var(--border-default)]">
              <Image src={masterUrl || "/seq-poster.svg"} alt="Main Storyboard" fill className="object-cover" />
            </div>
          ...
          <Card className="p-6 bg-[var(--surface-2)] border-[var(--border-default)] space-y-4">
            <div className="space-y-2">
              <Label>Describe the transition frames needed</Label>
              <Textarea
      ...
      {generatedUrl && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          <Card className="p-4 bg-[var(--surface-1)] border-[var(--border-default)]">
            <div className="aspect-3/2 relative rounded-lg overflow-hidden border border-[var(--border-default)]">
              <Image src={generatedUrl || "/seq-poster.svg"} alt="Transition Master" fill className="object-cover" />
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-[var(--text-primary)] font-medium flex items-center gap-2">
                    <Loader2 className="w-3 h-3 animate-spin" />
                    Preparing {panels.length + 1}/{analyzedCount}...
                  </span>
                </div>
